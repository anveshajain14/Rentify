        ...
        <div className="grid lg:grid-cols-2 gap-16">
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
            <div className="mb-6 flex items-center gap-2">
              <span className="px-4 py-1.5 bg-emerald-100 text-emerald-700 rounded-full text-sm font-bold">{product.category}</span>
              <button
            ...
            <h1 className="text-4xl font-black tracking-tighter text-black mb-4">{product.title}</h1>
            <div className="flex items-center gap-6 text-gray-500 mb-6">
              <span className="flex items-center gap-1 text-amber-500"><Star size={18} fill="currentColor" /> <span className="text-black font-bold">4.9</span> <span className="text-sm">(24 reviews)</span></span>
            ...
            {/* Seller block */}
            <Link href={`/seller/${product.seller?._id}`} className="flex items-center justify-between p-6 bg-gray-50 rounded-3xl mb-8 border border-gray-100 hover:border-emerald-500/50 transition-all group">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h4 className="font-bold text-black group-hover:text-emerald-600">{product.seller?.name}</h4>
                  <p className="text-xs text-gray-500">Member since {new Date(product.seller?.joinedAt).getFullYear()}</p>
                </div>
              </div>
              <span className="px-4 py-2 bg-white rounded-xl text-xs font-bold shadow-sm">View Shop</span>
            </Link>
            ...
            <div className="flex gap-3 mb-6">
              {[
                ...
                <div key={d.key} className="flex-1 p-4 rounded-2xl bg-gray-50 border border-gray-100">
                  <p className="text-xs font-bold text-gray-400 uppercase">{d.label}</p>
                  <p className="text-xl font-black text-black">${d.value}</p>
                </div>
              ))}
            </div>
            ...
            {availability.length > 0 && (
              <div className="mb-8 p-4 rounded-2xl bg-amber-50/80 border border-amber-100">
                <p className="text-sm font-bold text-amber-800 mb-2 flex items-center gap-2"><Calendar size={16} /> Blocked dates</p>
            ...
            {/* Booking card */}
            <div className="bg-black text-white p-8 rounded-[40px] shadow-2xl shadow-emerald-900/20 relative overflow-hidden">
              ...
              <button onClick={handleBooking} disabled={bookingLoading} className="w-full py-4 bg-emerald-600 text-white rounded-2xl font-black hover:bg-emerald-500 flex items-center justify-center gap-2 mb-4">
              ...
              <p className="text-center text-[10px] text-gray-500 mt-4 uppercase tracking-widest font-bold">Secure checkout with Stripe</p>
            </div>
          </motion.div>
        </div>
